...
public final class DateUtil {
    ...
    public static final String[] YEAR_MONTH_FORMAT = new String[]{
            "yyyy-MM-dd", "yyyyMMdd", "yyyy.M.d", "yyyy.MM.dd", "yyyy-M-d",
            "yyyy/MM/dd", "yyyy/M/d", "*yyyy/M/d", "*yyyy/MM/dd", "yyyy年M月d日", "yyyy年MM月dd日",
            "yyyy-MM", "yyyyMM", "yyyy/MM","yyyy.MM","yyyy年MM月","yyyy-M", "yyyy/M","yyyy.M","yyyy年M月"
    };
    ...
    public static String[] getFormatsForYearMonth() {
        LinkedHashSet<String> list = new LinkedHashSet<>();
        for (String date : YEAR_MONTH_FORMAT) {
            for (String time : TIME_FORMATS) {
                list.add(date + " " + time);
            }
            list.add(date);
     ...
     * @return
     */
    public static Date getDateValueFromStringForYearMonth(String str) {
        if (StringUtil.isBlank(str)) {
            return null;
        } else {
            for (int i = 0; i < getFormatsForYearMonth().length; i++) {
                String f = getFormatsForYearMonth()[i];
                SimpleDateFormat sf = new SimpleDateFormat(f);
                sf.setLenient(false);
                try {
                    return sf.parse(str);
     ...
     * @return
     */
    public static Date getDateValueFromStringForBatchUpload(String str) {
        if (StringUtil.isBlank(str)) {
            return null;
        } else {
            for (int i = 0; i < getBatchUploadFormats().length; i++) {
                String f = getBatchUploadFormats()[i];
                SimpleDateFormat sf = new SimpleDateFormat(f);
                sf.setLenient(false);
                try {
                    return sf.parse(str);
    ...
    /**
     * 获取给定yearmo的Date的当天开始时间戳
     *
     * @return
     */
    public static long getBeginTimestampByYearmoday(Integer yearmoday) {
        int day = yearmoday % 100;
        int mon = (yearmoday / 100) % 100;
        int year = yearmoday / 10000;
        Calendar cal = Calendar.getInstance();
    ...
    /**
     * 获取给定yearmo的Date的当天结束时间戳
     *
     * @return
     */
    public static long getEndTimestampByYearmoday(Integer yearmoday) {
        int day = yearmoday % 100;
        int mon = (yearmoday / 100) % 100;
        int year = yearmoday / 10000;
        Calendar cal = Calendar.getInstance();
     ...
     * @return
     */
    public static boolean checkYearFromDate(Date date){
        if(date == null){
            return false;
        }
        Integer year = DateUtil.getYearFromDate(date);
        return year < 10000 && year > 999;
    }
    ...
    public static Integer getMoFromDate(Date date) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
...
//        cal1.clear();
    ...
    public static Date getAttendanceDateByTimeStamp(int timeStamp) {
        Date clockDate = null;
        java.util.Date clockTimeDate = DateUtil.convertTimestampSecondsToDate(timeStamp);

        java.util.Date startTime = DateUtil.getDateOnHour(clockTimeDate, 4);
        java.util.Date endTime = DateUtil.getDateNextDayOnHour(clockTimeDate, 4);
     ...
     * @return
     */
    public static int getTimesByAttendanceDate(Date date) {
        return (int) (getDateStart(date).getTime()/1000);
    }
    ...
    public static Date getBatchTimeFromStr(String str) {
        ...
        throw new RuntimeException("操作失败，日期填写错误str:" + str);
    }
     ...
     * @return
     */
    public static Integer validateAndGetYearmoInt(String yearmo) {
        // 6位数字
        if (TimeConstant.DIGITS_OF_YEARMO != yearmo.length()) {
            throw new RuntimeException("yearmo不规范：" + yearmo);
        }

        try {
            return Integer.valueOf(yearmo);
        } catch (NumberFormatException e) {
            throw new RuntimeException("yearmo无法解析:" + yearmo);
        }
    }
    ...
    /**
     * 返回日期格式字符串的"MM月DD日"格式
     * @param date 格式：yyyyMMDD
     * @return
     */
    public static String getMonthAndDayByDate(String date) {
        return Integer.valueOf(date.substring(4,6)) + "月" + Integer.valueOf(date.substring(6)) + "日";
    }


    public static void main(String[] args) throws InterruptedException {
        Date startTime = DateUtil.convertTimestampSecondsToDate(1528023600);
    ...
    public static long getTimestampByDateAndTime(String dateKey,String time){
        Date date = getDateByFormatFromString(dateKey, "yyyyMMdd");
    ...
    public static int getAddTimestampBytime(long time,int field,int amount){
        Calendar cal = Calendar.getInstance();
        cal.setTimeInMillis(time);
        cal.add(field,amount);
        long timeInMillis = cal.getTimeInMillis();
        return new Long(timeInMillis/1000).intValue();
    }
     ...
     * 判断年份是否在当前年份之前
     */
    public static boolean isBeforeNowYear(Date date) {
     ...
     * @return
     */
    public static Date getNowDateForMD (Date date) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
    ...
    public static int getHoursOfUnixTimestamp(int time) {
        Calendar cal = Calendar.getInstance();
        cal.setTimeInMillis(new Long(time)*1000);
        int hour = cal.get(Calendar.HOUR_OF_DAY);
        return hour;
    }

    public static int getDateKeyFromUnixTimestamp(int time){
        Calendar cal = Calendar.getInstance();
        long longTime = new Long(time);
        cal.setTimeInMillis(longTime*1000);
        Date date = cal.getTime();
        String yyyyMMdd = getStringDateByFormat(date, "yyyyMMdd");
